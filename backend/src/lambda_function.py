...
def polygon_api_get_ticker_dividends(ticker: str) -> Any:
    response = requests.get(
        url=f"{POLYGON_HOST}/v3/reference/dividends",
...
def polygon_api_get_ticker_previous_close(ticker: str) -> Any:
    response = requests.get(
        url=f"{POLYGON_HOST}/v2/aggs/ticker/{ticker}/prev",
        headers=get_polygon_headers(),
    )
...
def polygon_api_get_ticker_details(ticker: str) -> Any:
    response = requests.get(
        url=f"{POLYGON_HOST}/v3/reference/tickers/{ticker}",
        headers=get_polygon_headers(),
    )
    return response.json()["results"]
...
def polygon_download_ticker_logo(ticker: str, path: Path) -> Path:
    ...
    response = requests.get(
        url=details["branding"]["logo_url"], headers=get_polygon_headers()
    )
